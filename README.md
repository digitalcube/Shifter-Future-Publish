...
プラグインを有効にすると、対象の投稿タイプで未来日を設定して保存した投稿は実際に「公開」ステータスになり、フロントエンド全体で閲覧可能になります。アーカイブや個別ページだけでなく、RSS/Atom フィードや公開 REST API（例: `/wp-json/wp/v2/posts`）にも含まれるため、匿名の訪問者や検索エンジンから即座に閲覧可能になります。公開日まで非公開にしたいコンテンツには未来日を設定しないでください。
...
### フィードや REST API にも未来日投稿は表示されますか？

はい。対象の投稿タイプの未来日投稿は即座に公開されるため、ほかの公開済み投稿と同様に RSS/Atom フィードや公開 REST API のレスポンス（例: `/wp-json/wp/v2/posts`）に含まれます。公開日まで秘匿すべきコンテンツには未来日を使用しないでください。
