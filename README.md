...
未来日の投稿を保存する際、`wp_insert_post_data` フィルターでデータベースに保存される前にステータスを「future」から「publish」に変更します。シンプルなアプローチで確実に未来日投稿を公開状態にします。
...
- 大規模リファクタリング: 冗長な保護レイヤーを削除しコードを簡略化
- PHP を 568 行から約 250 行に削減
- JavaScript ファイルを大幅に簡略化
- 削除: 複数のフォールバック機構（MutationObserver、setInterval、複数イベントリスナー）
- 維持: ステータス変更用の wp_insert_post_data フィルター
- 維持: Gutenberg ボタンテキスト用の wp.data.subscribe
- 維持: クラシックエディタボタンテキスト用の jQuery change イベント
- パフォーマンス: 同じ機能でより軽量なフットプリント
