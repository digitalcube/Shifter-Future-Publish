...
if ( ! defined( 'SHIFTER_FUTURE_PUBLISH_VERSION' ) ) {
	define( 'SHIFTER_FUTURE_PUBLISH_VERSION', '2.1.4' );
}
...
final class Shifter_Future_Publish {
	...
	private function load_settings(): void {
		...
		$saved          = get_option( 'shifter_future_publish_settings', array() );
		$saved_array    = is_array( $saved ) ? $saved : array();
		$parsed         = wp_parse_args( $saved_array, $defaults );
		$this->settings = array(
			'enabled'    => (bool) ( $parsed['enabled'] ?? true ),
			'post_types' => is_array( $parsed['post_types'] ?? null ) ? array_values( array_filter( $parsed['post_types'], 'is_string' ) ) : array( 'post' ),
		);
	}
	...
	public function register_settings(): void {
		register_setting(
			...
			array( 'sanitize_callback' => $this->sanitize_settings( ... ) )
		);
